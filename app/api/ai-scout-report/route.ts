...
ตอบกลับเป็น JSON เท่านั้น (ไม่มี markdown, ไม่มี code block) ในรูปแบบ:
{
  ...
  const MODELS = ['gemini-2.0-flash-lite', 'gemini-2.0-flash', 'gemini-2.0-flash-exp', 'gemini-1.5-flash-8b'];
  ...
  for (const modelName of MODELS) {
    ...
    try {
      ...
      if (res.status === 429) {
        // quota หมด — ไม่รอ เพราะ Vercel จะ timeout
        return NextResponse.json({
          error: '⚠️ Quota วันนี้หมดแล้ว\n\nวิธีแก้:\n1. รอพรุ่งนี้ (quota reset ทุกวัน)\n2. สร้าง Project ใหม่ใน aistudio.google.com แล้วได้ quota ใหม่ทันที\n3. เพิ่ม billing $5 ที่ aistudio.google.com เพื่อใช้แบบ unlimited',
        }, { status: 429 });
      }
