...
ตอบกลับเป็น JSON เท่านั้น (ไม่มี markdown, ไม่มี code block) ในรูปแบบ:
{
  ...
  for (const modelName of MODELS) {
    try {
      ...
      if (res.status === 404) continue; // ลอง model ถัดไป
      ...
      if (MODELS.indexOf(modelName) === MODELS.length - 1) {
        ...
        console.error('AI Scout Report error:', msg);
        return NextResponse.json({ error: msg }, { status: 500 });
      }
      // ยังมี model อื่นให้ลอง — continue
    }
  }

  return NextResponse.json({ error: 'No available Gemini model found for this API key' }, { status: 503 });
}
