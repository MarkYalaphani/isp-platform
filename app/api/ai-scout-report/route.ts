...
ตอบกลับเป็น JSON เท่านั้น (ไม่มี markdown, ไม่มี code block) ในรูปแบบ:
{
  ...
  try {
    ...
    const model = genAI.getGenerativeModel({
      ...
      systemInstruction: 'คุณเป็น Professional Football Scout ผู้เชี่ยวชาญนักเตะเยาวชนไทย ตอบเป็น JSON เท่านั้น ไม่มี text อื่น ไม่มี markdown code block',
    });
    ...
    console.error('AI Scout Report error:', err);
