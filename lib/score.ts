...
export function getScorePoint(metric: string, val: string | number, dob: string): number {
  ...
  // ─────────────────────────────────────────────────────────────────
  // U12 : อายุ 8–12 ปี
  // เกณฑ์ผ่อนปรนตามพัฒนาการ ไม่ต้องการพละกำลังเท่าผู้ใหญ่
  // ─────────────────────────────────────────────────────────────────
  if (age >= 8 && age <= 12) {
    switch (metric) {
      case 'speed30':
        // ดีมาก: <5.0s  ดี: 5.0–5.4  ปานกลาง: 5.4–5.8  พัฒนา: 5.8–6.2  ต้องปรับ: >6.2
        return v <= 5.00 ? 5 : v <= 5.40 ? 4 : v <= 5.80 ? 3 : v <= 6.20 ? 2 : 1;

      case 'cmj':
        // ดีมาก: ≥32cm  ดี: 27–32  ปาน: 22–27  พัฒนา: 17–22  ต้อง: <17
        return v >= 32 ? 5 : v >= 27 ? 4 : v >= 22 ? 3 : v >= 17 ? 2 : 1;

      case 'agility':
        // Illinois Agility Test  ดีมาก: ≤18.5s
        return v <= 18.50 ? 5 : v <= 20.00 ? 4 : v <= 21.50 ? 3 : v <= 23.00 ? 2 : 1;

      case 'situp':
        // 40s sit-up  ดีมาก: ≥28
        return v >= 28 ? 5 : v >= 23 ? 4 : v >= 18 ? 3 : v >= 13 ? 2 : 1;

      case 'pushup':
        // 40s push-up  ดีมาก: ≥18
        return v >= 18 ? 5 : v >= 14 ? 4 : v >= 10 ? 3 : v >= 6 ? 2 : 1;

      case 'longjump':
        // Standing broad jump  ดีมาก: ≥175cm
        return v >= 175 ? 5 : v >= 158 ? 4 : v >= 141 ? 3 : v >= 124 ? 2 : 1;

      case 'yoyo':
        // Yo-Yo IR1 — เด็กวิ่งน้อยกว่าผู้ใหญ่มาก (Rebelo 2014)
        // ดีมาก: ≥840m  ดี: 640  ปาน: 400  พัฒนา: 200
        return v >= 840 ? 5 : v >= 640 ? 4 : v >= 400 ? 3 : v >= 200 ? 2 : 1;

      case 'sitreach':
        // Sit & Reach  ดีมาก: ≥20cm
        return v >= 20 ? 5 : v >= 14 ? 4 : v >= 8 ? 3 : v >= 2 ? 2 : 1;
    }
